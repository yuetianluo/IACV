...
parfor exp_index = 1:exp_num
	...
	pi = (1./(1+exp(-X * mu)));
	y = binornd(1,pi);
	final_error_matrix{exp_index} = comparison_proxgd(X, y, mu, n, p, iter_max, lambda, alpha_0);
end
...
% output
filename = join(["Prox_GD_experiement","p", join(string(p),'_'), "p_star", join(string(p_star),'_'),  "n", join(string(n), '_'),"iter_max",  join(string(iter_max),'_'), "lambda", lambda, "step-size", alpha_0,'exp_num', exp_num],'_');
