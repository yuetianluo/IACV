...
%  mu = randn(p,1);
  ...
  pi = (1./(1+exp(-X * mu)));
  y = binornd(1,pi);
  final_error_matrix{exp_index} = comparison_sgd(X, y, mu, n, p, M, step_size, iter_max, lambda, alpha_0);
end
...
% output
filename = join(["SGD_experiement","p", join(string(p),'_'), 'p_star', p_star, "n", join(string(n), '_'),"iter_max",  join(string(iter_max),'_'), "lambda", join(string(lambda),'-'),  "batch_size", join(string(M),'-'), "step-size", step_size, 'alpha0', alpha_0, 'exp_num',exp_num  ],'_')
